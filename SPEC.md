...
- Show opens: **shipped** — a tune-in after ≥`KALLIOPE_SHOW_OPEN_QUIET_MIN` (45) minutes of empty listenership wakes the DJ with an occasion note; phrasing is tape-delay-safe ("write so it survives if they aren't there"). The quiet-gap requirement is itself the cooldown.
- DJ memory: **CSPN-style chronicle shipped** (§1.6) — `events` fold into first-person chapters (Cal writes them as memories, thin stretches get a canned stub, no API call), chapters cascade-merge 6→1 into arcs; state docs carry "the story so far" under a ~600-token budget. Maintenance rides the planning cadence. Tables `chronicle`/`chronicle_meta`, kalliope-owned.
